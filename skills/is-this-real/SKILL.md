...
- Prefer specific nouns over generic or broad labels such as `artifact`, `layer`, `framework`, or `seam`.
- Separate facts, inferences, assumptions, and unknowns.
- Recommend the smallest change that solves the real problem.
- Determine the domain from the reviewed work, the user's request, and verified repository context.
- Treat repository terms and structure as claims to check, not rules to follow.
- Use repository context only when the reviewed work clearly belongs to that repository.
...
2. Describe the behavior in literal terms without reusing the repository's disputed labels.
...
5. Classify how the implementation fits the concept:
   ...
   - **Partial**: It implements some defining properties and names the differences.
   - **Mismatch**: The name promises behavior the implementation does not provide.
...
Research authoritative external sources before concluding that a term, method, metric, architecture, or practice is established, standard, or widely adopted.
Research the defining properties before concluding that the implementation matches an established concept.
...
- Do not infer an established concept from similar words or superficial structure.
- Do not claim that a concept is nonexistent only because authoritative evidence was not found.
...
- Do not confuse proxy, synthetic, or weak labels with direct evidence of the target outcome.
